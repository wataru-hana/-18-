...
def normalize_material(material_name):
    ...
    return normalized_material, best_match_length
...
for material, expected in test_cases:
    result, match_len = normalize_material(material)
    status = '✓' if result == expected else '✗'
    if result != expected:
        failed.append((material, result, expected))
    print(f'{status} {material:25} -> {result:20} (期待: {expected:20}) [マッチ長: {match_len}]')

print()
print('=' * 70)
if failed:
    print(f'失敗: {len(failed)}件')
    for material, result, expected in failed:
        print(f'  {material} -> {result} (期待: {expected})')
else:
